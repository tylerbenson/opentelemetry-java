...
final class ParentExcludedSpanLinks implements SpanLinks {
  ...
  static SpanLinks create(SpanContext parentSpanContext) {
    return new ParentExcludedSpanLinks(parentSpanContext);
  }

  static SpanLinks create(SpanContext parentSpanContext, SpanContext firstLink) {
    if (parentSpanContext.equals(firstLink)) {
      return new ParentExcludedSpanLinks(parentSpanContext);
    }
    return new ParentExcludedSpanLinks(
        parentSpanContext, new LinkedHashSet<>(Collections.singleton(firstLink)));
  }
  ...
  @Override
  public ImplicitContextKeyed updateParent(SpanContext spanContext) {
